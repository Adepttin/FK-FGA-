...
//class for DF ph ladder calculation based on DMFT results
...
//calculate vertex function F as given in ph ladder
...
//write result to ohm
template <typename numbertype , typename fltype>
int calcconohm(const numbertype* const* const Fqup, const numbertype* const Gk, const numbertype* const Matsus, const fltype* const vx, const int nv2, const int nw2, const int nk, const fltype beta, numbertype* const ohm , const int *const ktoq, const int* const * const ksum)
{
		...
		*(ohm + i) = 0.;
		...
		for(qind = 0; qind < norm; qind++) //q
		{
			...
			for(k = 0; k < norm; k++)
			{
				...
				Casym = Fqup[qredind][(nv2-1)*(2*nv2)-nv2];
				...
				for(v = -nv2; v < nv2-i; v++) //v
				{
					...
					dumg = dumg*dumg;
					//G(k,v)*G(k+q,v)*G(k+q,v + w)*G(k,v + w)
					...
					dummyb += ( (Fqup[qredind][(v)*(2*nv2+1)+i] * dumg2) - (Casym * dumg) ) * vx[k] * vx[kp];
				}
...
class ConductivityObject
{
	...
	int CalcCondVertex()
	{
		calcconohm(Fup, Gk, Matsus, vx, nv, nv, nk, beta, ohmvertex, ktoq, ksum);
		return(0);
	}
